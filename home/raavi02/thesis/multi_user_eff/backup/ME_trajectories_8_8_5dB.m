...
for l=1:N
    mf(:,l)=pn_code((l-1)*sf+1:l*sf,:)'*tx((l-1)*sf+1:l*sf)/(sf*sqrt(2));
end
...
axis([0 1 0 1])
xlabel('\eta^{(m-1)} \rightarrow')
ylabel('\eta^{(m)} \rightarrow')
hold on
