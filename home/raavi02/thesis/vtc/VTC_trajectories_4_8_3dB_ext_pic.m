...
y_ax=test(:);
cc = plot(x_ax,y_ax);
dd = plot(x_ax,y_ax,'marker','x');
grid
sigma_d_ic=0:0.1:1;
alpha=(user-1)/sf;
sigma_eff_ic=sigma2/(2*sf)+alpha*sigma_d_ic;
aa = plot(sigma_d_ic,sigma_eff_ic,'r')
filename=['sigma_d_vs_sigma_eff' num2str(bi2de(g(1,:))) num2str(bi2de(g(2,:))) '.mat'];
load(filename)
bb = plot(sigma_d_cc,sigma_eff_cc,'k')
legend('aaaaaaaaa','bbbbbbbbb','location','northeastoutside');
axis([0 1 0 1]);
hold off
xlabel('x')
ylabel('y')
...
print -deps VTC_trajectories_4_8_3dB_ext_pic_psfrag.eps
