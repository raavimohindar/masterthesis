...
u = (sign(randn(n_bits,1))+1)/2;

val = n_bits / k1 ;

for start=0:val-1
    c((start*n1)+[1:n1]) = mod(u((start*k1)+[1:k1])'*g,2);
end
enc = 1-2*c;

EsN0dB = 10;

snr = 10.^((EsN0dB/10)*(4/7));

sigma = 1/sqrt(snr*2);

dual = 0;

noise = randn(val*n1,1).*sigma;
...
if ( dual == 1)
  ...
  metric(1:k,2) = metric(1:k,2) .* exp(L_a) ./ (1 + exp(L_a));  % considering a-priori information only for infobits
end
