/* Function file: SS-MAX-Log-MAP-Decoding of RSC and NSC Convolutional Codes
 * Copyright 1998 Volker Kuehn and Armin Dekorsy
 ...
 * DESCRIPTION:
 * -------------------------------------------------------------------------------------------------
 * 1. SS-Max-Log-MAP-Decoding of Convolutional Codes supplying LLR's of information bits
 *    and coded bits (optional)
 * 2. Suitable for NSC and RSC Convolutional Codes
 * -------------------------------------------------------------------------------------------------
 * INPUT:
 * -------------------------------------------------------------------------------------------------
 * signal:    struct containing the following fields
 *                signal.sig        : input LLR's from matched filter output
 *                signal.L_a        : a priori information for each information bit
 *                signal.last_state : last state of trellis if code.term==1
 *
 ...
 * L_code           LLR's for codebits (optional)
 * -------------------------------------------------------------------------------------------------
 * FUNCTIONS:
 * -------------------------------------------------------------------------------------------------
 ...
 *         last changes by Volker Kuehn, 03.04.01, 29.08.2001
 * * -------------------------------------------------------------------------------------------------
 */
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  double *L_info;        /* Zeiger auf Decodierergebnis */
 ...
 *------------------------------------------------------------------------ */
      for (i=0; i<num_word; i++) {
          ...
          metric[i] = 0.0;
          for (j=0; j<n; j++) {
               metric[i] -= MAX( ((m & 1) ? *(sig+j) : -*(sig+j)),0 );
               m >>= 1;
          }
      }
 ...
 *------------------------------------------------------------------------ */
      for (m=0; m<num_state; m++) {
        *(gamma0+m*block_len+k) = metric[(int)trellis_out[m]] - MAX(0,-*Z);
        dummy = *(gamma0+m*block_len+k) + *(alpha+m*(block_len+1)+k);
        *(alpha+((int)trellis_next[m])*(block_len+1)+k+1) = MAX( *(alpha+((int)trellis_next[m])*(block_len+1)+k+1), dummy );
        norm[k] = MAX( norm[k], dummy);
 
        *(gamma1+m*block_len+k) = metric[(int)trellis_out[m+num_state]] - MAX(0,*Z);
        dummy = *(gamma1+m*block_len+k) + *(alpha+m*(block_len+1)+k);
        *(alpha+((int)trellis_next[m+num_state])*(block_len+1)+k+1) = MAX( *(alpha+((int)trellis_next[m+num_state])*(block_len+1)+k+1), dummy );
        norm[k] = MAX( norm[k], dummy );
      }
 ...
 *--------------------------------------------------------------------------------------------*/
  for (k = block_len-1; k > 0; k--) {
      for (m = 0; m < num_state; m++) {
          *(beta+m*block_len+k-1) = *(gamma0+m*block_len+k) + *(beta+((int)trellis_next[m])*block_len+k);
          *(beta+m*block_len+k-1) = MAX( *(beta+m*block_len+k-1), *(gamma1+m*block_len+k) + *(beta+((int)trellis_next[m+num_state])*block_len+k)) - norm[k];
      }
  }
 ...
 *---------------------------------------------------------------------------------------------*/
    ...
    for (m = 0; m < num_state; m++) {
      L0 = MAX( L0, *(alpha+m*(block_len+1)+k) + *(gamma0+m*block_len+k) + *(beta+((int)trellis_next[m])*block_len+k) );
      L1 = MAX( L1, *(alpha+m*(block_len+1)+k) + *(gamma1+m*block_len+k) + *(beta+((int)trellis_next[m+num_state])*block_len+k));
    }
    L_info[k] = L0 - L1;
...
/*---------------------------------------------------------------------------------------------
 * Calculating log-likelihood-ratio for coded bits for each time instant k
 * 
 * For each codebit the LLR`s to be 1 or 0 are evaluated by the probibilities of the
 ...
 *    alpha1 as well as gamma_i (i=0,1) and beta
 *
 * Due to foregoing states: alpha_j(m,k) or beta(next state,k) and gamma_j(m,k) are used
 *---------------------------------------------------------------------------------------------*/

       for (j=0;j<n;j++) {     /*Loop over codebits*/

         L0 = -1.0e30;
         L1 = -1.0e30;

     for (m = 0; m < num_state; m++) {      /*Loop over states*/
           ...
           if ((out_info0 & 1) == 0)     /*Infobit=0 Codebit=0*/
             ...
             L1 = MAX(L1, (*(alpha+m*(block_len+1)+k) + *(gamma0+m*block_len+k) + *(beta+((int)trellis_next[m])*block_len+k) ));

           if ((out_info1 & 1) == 0)     /*Infobit=1 Codebit=0*/
             L0 = MAX(L0, (*(alpha+m*(block_len+1)+k) + *(gamma1+m*block_len+k) + *(beta+((int)trellis_next[m+num_state])*block_len+k)) );
