% ##############################################################################
% ##  ber_dpsk_koh_awgn.m : BER fuer DPSK beim AWGN Kanal,                    ##
% ##                        kohaerente Detektion                              ##
% ##############################################################################
%
% function [meanber, ber, ser, biterror, bitcount, symerror, symcount] ...
%  = ber_dpsk_koh_awgn(EbN0db,M,map,maxbiterror,maxbitcount)
% ------------------------------------------------------------------------------
% EINGABE:
...
K=log2(M);
if (K~=fix(K))
  disp(['# ',mfilename,'  M ist keine Potenz von 2.']);
end;
