...
na
    ...
    for l=1:N
        ...
        for n=1:user
            ind=[1:n-1,n+1:user];
            out(n,l)=4*EsN0(loop)*(mf(n,l)-R_xx(n,ind)*clipping(tanh(L_a(l,ind).'/2),0.4,0));
        end
    end
    for n=1:user
        signal.sig=L_a(:,n);
...
plot(I_e_cc,I_a);
filename=['exit_chart_' num2str(EsN0_dB) 'dB_' 'generic_PIC' num2str(user) 'user']
